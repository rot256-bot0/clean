...
class DesignTests(unittest.TestCase):
    def test_caliper_runtime_section_and_exact_excerpts(self):
        ...
        self.assertIn("theorem modMul_runtime", text)
        self.assertIn("theorem modMul_exact_cost", text)
        self.assertIn("not a Rust runtime", text)
        self.assertIn("mem.alloci b0, 6", text)
        self.assertIn("witgen [x, y] do", text)
        self.assertIn("fields![square := square, output := output]", text)
        self.assertIn("witgen_native::Bn254Scalar", text)
        self.assertIn("pub type Result<T> = std::result::Result<T, Error>", text)
        self.assertIn("def lowerSplit", text)
        self.assertNotIn("witgen_native::F17", text)
