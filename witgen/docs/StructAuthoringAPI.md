...
`witgen [names...] do` names inputs in declared context order. Supported forms include named operation binds, reference-alias lets, returns, `if`/`else`, and exhaustive Option matches with `none` and `some` branches. References are weakened through every operation binding. The branching surface elaborates to typed regions with explicit capture lists; payload patterns bind fresh variables and preserve lexical shadowing. Conditional branching requires `BranchOp`; Option construction/case analysis uses `ValueOp`. Explicit nested `witgen` regions remain available for other region operations. Pure non-reference lets and unsupported statements are rejected rather than assigned guessed weakening semantics. Smart wrappers hide core constructor plumbing. See [BranchAPI](BranchAPI.md).
...
- The library's `CurveId` descriptor, only after its actual constructor is checked to contain one String and seven Fin fields. A name alone does not grant this exception; arbitrary records remain rejected.
